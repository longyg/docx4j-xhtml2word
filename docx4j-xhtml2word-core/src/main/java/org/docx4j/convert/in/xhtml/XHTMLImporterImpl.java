...
public class XHTMLImporterImpl implements XHTMLImporter {
    ...
    /**
     * @param runFormatting the runFormatting to set
     */
    ...
    /**
     * @param paragraphFormatting the paragraphFormatting to set
     */
...
//		FontHandler.setRFont(fontFamily, rPr );
    ...
    private void setInlineFont(RPr rPr, Map<String, PropertyValue> cssMap) {
        ...
        for (Map.Entry<String, PropertyValue> entry : cssMap.entrySet()) {
            ...
            String styleValue = entry.getValue().getStringValue();
            if (StyleHelper.isFontStyle(styleName)) {
                if (rFonts == null) {
                    rFonts = Context.getWmlObjectFactory().createRFonts();
                }
                try {
                    setFontField(rFonts, styleName, styleValue);
                } catch (IllegalAccessException e) {
                    log.warn("Failed to set font field value for: {}, {}", styleName, styleValue, e);
                }
            }
        }
        if (null != rFonts) {
    ...
    private void setFontField(RFonts rFonts, String fieldName, String value) throws IllegalAccessException {
        Field[] declaredFields = RFonts.class.getDeclaredFields();
